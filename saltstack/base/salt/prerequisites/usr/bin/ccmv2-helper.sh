setup_ccmv2() {
  ...
  : ${CCM_V2_IV:=436c6f7564657261436c6f7564657261}
  ...
  echo ${CCM_V2_AGENT_ENCIPHERED_KEY} | openssl enc -aes-128-cbc -d -A -a -K ${CCM_V2_AGENT_KEY_HEX} -iv ${CCM_V2_IV} > ${AGENT_KEY_PATH}
  ...
  if [[ "$IS_CCM_V2_JUMPGATE_ENABLED" == "true" && "$IS_FREEIPA" == "true" ]]; then
    ...
    ACCESS_KEY="$(echo ${CCM_V2_AGENT_ENCIPHERED_ACCESS_KEY} | openssl enc -aes-128-cbc -d -A -a -K ${CCM_V2_AGENT_KEY_HEX} -iv ${CCM_V2_IV})"
  fi
